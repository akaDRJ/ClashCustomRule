...
// ============================== 可调总开关 ==============================
const TUNING = {
  MIN_NODES_PER_COUNTRY: 2,          // 每个国家至少命中多少节点才生成该国家分组
  ENABLE_EMPTY_GROUP_CLEANUP: true,  // 生成后是否清理空组（建议保留 true）
  DEBUG_REPORT: false,               // 打开后在 config.__debug 输出识别统计
  WORD_BOUNDARY_FOR_ISO2: true       // 仅作规范提示，真实边界在 countryRegex 中体现
};
// ======================================================================
\n\n/*
...
// Emoji 国旗 → ISO2 转换，以及 ISO2 → 国家中文名映射
function flagToISO2(flag) {
    ...
    for (const ch of Array.from(flag)) {
        ...
        if (cp >= 0x1F1E6 && cp <= 0x1F1FF) {
            const A = 0x1F1E6;
            const letter = String.fromCharCode(65 + (cp - A)); // 65 = 'A'
            codes.push(letter);
        }
    }
